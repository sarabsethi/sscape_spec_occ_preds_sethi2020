...
'''
Split data into K folds, and in each fold fit GMMs to training acoustic feature data
'''
...
for spec_type in all_spec_types:
    for pc_data_fname in all_pc_data_fs:
        ...
        # Load point count data with audio features
        ...
        # Load lists of point counts and species
        if spec_type == 'herp':
        ...
        # Groups are sites for the leave one group out cross-validation
        ...
        for species in chosen_specs:
            # Create an array of labels (1 is present, 0 is absent)
            ...
            # Perform leave on group out cross validation (in this case groups are sites)
            cval = LeaveOneGroupOut()

            for k, (train_index, test_index) in enumerate(cval.split(pcs_feats, pcs_spec_labs, all_pcs_groups)):
                # Allows runs to be processed one at a time in parallel (e.g., using HPC)
                run = run + 1
                ...
                # Fit a Dirichlet-Process GMM to the training data
                ...
                # Fit a normal GMM to the training data
