...
'''
Plot a (site x time) matrix comparing how species occurrence predictions from
soundscapes compare to the true occurrence of each species
'''
...
# If only showing a selection of species (in main text) plot all in same figure
...
for spec_type in all_spec_types:
    # Load classification results for species type
    ...
    # Sort species by descending AUC
    spec_sort_ix = np.argsort(np.argsort(aucs)[::-1])

    # If showing all species (supp figure) plot each species type in a new figure
    if show_all_specs:
        n_cols = 3
    ...
    for spec_ix, spec in enumerate(auc_specs):
        ...
        # Get a unique list of sites from the classification results
        ...
        # Ensure arrays are nparrays (rather than lists)
        ...
        # Go through each site extracting classification scores based on soundscape data, and true labels based on the point counts
        for site_ix, site in enumerate(unq_sites):
            ...
            # Go hour by hour to fill each element of the plotted (site x time) matrix
            for hr in all_hrs:
        ...
        # Sort sites by increasing AGB (from oil palm -> old growth)
        ...
        # Get the 50th percentile for the bottom of the colour bar
        ...
        # Plot on top of the audio predictions the true occurrence labels (as red dots)
