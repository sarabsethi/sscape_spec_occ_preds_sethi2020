...
class Site:
    ...
    def get_agb(self, average_size='100m'):
        # Get AGB of the site using a hardcoded lookup approach
        # NOTE: Actually ACD (not AGB) but naming left to avoid full refactor of codebase

        # mean_agbs_marion = np.asarray([0.270780356, 0.674964289, 0.714754937, 1.103958403, 2.071538492, 2.381011351, 2.542379803, 2.707194682, 2.854348647, 2.854348647, 3.017740631, 3.566314216, 6.804037942, 6.804037942])
        mean_acds_100m_lidar = np.asarray([1.83033822290599, 18.362848997116, 9.19284311930338, 7.9858676592509, 9.01255162556966, 26.0916322072347, 52.336290359497, 19.7272520065307, 57.4237375259399, 110.205904642741, 55.5224819183349, 45.3377046585083, 109.960461934408, 103.380162556966])
        mean_acds_250m_lidar = np.asarray([0.857147406077101, 17.9305426597595, 14.4384284270437, 14.4384284270437, 14.4384284270437, 33.842208184694, 32.887174084073, 19.1690843732733, 62.6292404901414, 93.0919998570492, 47.215900220369, 61.2401834487915, 91.7128396285208, 115.545213222503])
        mean_acds_500m_lidar = np.asarray([0.763847697667683, 17.5877056483981, 21.5015854775151, 19.6793184220036, 17.9977403940299, 27.5453119950417, 33.8082380417066, 20.223447004954, 63.2861153873396, 87.0486274865957, 56.0953645706176, 61.6579288289517, 84.4472413124976, 121.665579863957])

        if average_size == '100m': mean_acds = mean_acds_100m_lidar
        elif average_size == '250m': mean_acds = mean_acds_250m_lidar
        elif average_size == '500m': mean_acds = mean_acds_500m_lidar
        else: raise Exception('average_size parameter to get_agb() not recognised')
        ...
        agb = mean_acds[site_names == self.name]
